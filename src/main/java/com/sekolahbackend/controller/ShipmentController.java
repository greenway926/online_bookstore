...
public class ShipmentController {
    ...
    @PostMapping("/create")
    public ShipmentModel create(@RequestBody @Valid ShipmentCreateRequestModel request, BindingResult result, HttpServletResponse response) throws IOException {
        ShipmentModel shipmentModel = new ShipmentModel();
        if (result.hasErrors()) {
            response.sendError(HttpStatus.BAD_REQUEST.value(), result.getAllErrors().toString());
            return shipmentModel;
        } else return shipmentService.create(request);
    }

    @PostMapping("/update")
    public ShipmentModel update(@RequestBody @Valid ShipmentUpdateRequestModel request, BindingResult result, HttpServletResponse response) throws IOException {
        ShipmentModel shipmentModel = new ShipmentModel();
        if (result.hasErrors()) {
            response.sendError(HttpStatus.BAD_REQUEST.value(), result.getAllErrors().toString());
            return shipmentModel;
        } else return shipmentService.update(request);
    }
